defmodule ProGen.ConfigTest do
  use ExUnit.Case
  ...
  describe "read/0" do
    test "returns defaults when no config file exists" do
      assert {:ok, %{use_conventional_commits: false}} = ProGen.Config.read()
    end

    test "reads .progen.yml with use_conventional_commits: true" do
      File.write!(".progen.yml", "use_conventional_commits: true\n")
      assert {:ok, %{use_conventional_commits: true}} = ProGen.Config.read()
    end
    ...
    test "returns error for malformed YAML" do
      ...
      assert {:error, msg} = ProGen.Config.read()
  ...
  describe "use_conventional_commits?/0" do
    test "returns false when no config file" do
      refute ProGen.Config.use_conventional_commits?()
    end

    test "returns true when enabled" do
      File.write!(".progen.yml", "use_conventional_commits: true\n")
      assert ProGen.Config.use_conventional_commits?()
    end
