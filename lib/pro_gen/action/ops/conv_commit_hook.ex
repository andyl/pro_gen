defmodule ProGen.Action.Ops.ConvCommitHook do
  @moduledoc """
  Install a Conventional Commits git hook in the current repository.

  ## Preconditions

  Requires `mix.exs` and `.git` to exist (validated via the `filesys` validator).

  ## What it does

  1. Creates a `.githooks/` directory with a `commit-msg` hook that validates
     commit messages against the Conventional Commits spec.
  2. Creates a `bin/install-git-hooks.sh` script that configures git to use
     the `.githooks/` directory.
  3. Appends a "Git Hooks" section to `README.md` with setup instructions
     (skipped if no README.md exists).
  4. Runs the install script to activate the hooks immediately.

  ## Skipped when

  The action is skipped (`needed?/1` returns `false`) when both
  `.githooks/commit-msg` and `bin/install-git-hooks.sh` already exist.

  ## Developer setup

  After cloning a repo with this hook installed, run once:

      ./bin/install-git-hooks.sh

  Then test with an intentionally bad commit message to verify enforcement.
  """
