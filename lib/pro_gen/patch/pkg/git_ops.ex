defmodule ProGen.Patch.Pkg.GitOps do
  @moduledoc """
  Mix.exs changes required by the 'git_ops' package.

  Learn more about GitOps config on the README.

  https://github.com/zachdaniel/git_ops#configuration

  This module performs two modifications:

  1. Add a `git_ops` element to the `project` keyword list,.
  2. Add a `git_ops` function to the `mix.exs` file.
  ...
  def install_config_block do
    :ok
  end

  def update_mix_version do
    :ok
  end
  ...
  The `value_code` parameter is a string of Elixir code (e.g., `"git_ops()"`)
  or `":my_app"`).
