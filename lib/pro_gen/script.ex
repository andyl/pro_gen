defmodule ProGen.Script do
  ...
  Functions for ProGen scripts.
    ...
    * `cli_args/1`    — Store an Optimus schema in ProGen.Script.Env
  ...
  @doc \"""
  Stores an Optimus schema in `ProGen.Script.Env` under `:pg_cli_args`.
  """
  def cli_args(schema) do
    ProGen.Script.Env.put(:pg_cli_args, schema)
  end
  ...
  """
  def cli_args do
    ProGen.Script.Env.get(:pg_cli_args)
  end
  ...
  """
  def cli_vals do
    ProGen.Script.Env.get(:pg_cli_vals)
  end
  ...
  a single flat map and stores it in `ProGen.Script.Env` under `:pg_cli_vals`.
  ...
  def parse_args(argv) when is_list(argv) or is_binary(argv) do
    ...
    case parse_args(schema, argv) do
      {:ok, parsed} ->
        ...
        ProGen.Script.Env.put(:pg_cli_vals, merged)
        {:ok, merged}
  ...
  defdelegate puts(text),          to: ProGen.Xt.PGIO
  defdelegate inspect(text, term), to: ProGen.Xt.PGIO
  defdelegate err(text),           to: ProGen.Xt.PGIO
  defdelegate log(text),           to: ProGen.Xt.PGIO
  defdelegate clear,               to: ProGen.Xt.PGIO
  ...
  def start(message \\ "START") do
    ...
    ProGen.Script.Env.put(:pg_start_time, now)
    ProGen.Script.Env.put(:pg_start_timestamp, timestamp)
    log(message)
  end
  ...
  def finish(message \\ "FINISH") do
    now = NaiveDateTime.local_now()
    timestamp = Calendar.strftime(now, "%y%m%d_%H%M%S")
    ProGen.Script.Env.put(:pg_finish_time, now)
    ProGen.Script.Env.put(:pg_finish_timestamp, timestamp)

    elapsed =
      case ProGen.Script.Env.get(:pg_start_time) do
        nil ->
          "00h 00m 00s"
    ...
    ProGen.Script.Env.put(:pg_elapsed_time, elapsed)
  ...
  defp auto_commit(desc, commit_type, opts) do
    ProGen.Xt.AutoCommit.auto_commit(desc, commit_type, opts)
  end
