...
**Behavior module:** `ProGen.Action` defines six callbacks, listed here in execution order: `opts_def/0` (NimbleOptions schema, args validated first), `depends_on/1` (dependencies run next), `needed?/1` (skip check), `validate/1` (precondition checks), `perform/1` (main execution), and `confirm/2` (postcondition). The `__using__` macro provides sensible defaults for all callbacks except `perform/1`, plus injected helpers `validate_args/1` (calls NimbleOptions) and an auto-generated `usage/0` (overridable).
...
**Adding a new action:** Create a module under `lib/pro_gen/action/` named `ProGen.Action.<Name>` that does `use ProGen.Action` and implements `perform/1` (required). Optionally override any other callback with `@impl true`: `opts_def/0` (NimbleOptions schema, default `[]`), `depends_on/1` (list of dependency action names or `{name, opts}` tuples), `needed?/1` (skip check, default `true`), `validate/1` (list of `{validator_name, checks}` tuples for precondition checks, default `[]`; e.g., `def validate(_args), do: [{"filesys", [:has_mix, :has_git]}]`), and `confirm/2` (postcondition, default `:ok`). It will be auto-discovered — no manual registration needed. Action names must be unique across all modules.
